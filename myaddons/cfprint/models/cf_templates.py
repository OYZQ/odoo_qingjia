...
class CFTemplate(models.Model):
    ...
    templ_historied = fields.One2many('cf.template.history', 'origin', string=u'History', help=u"History of template")
    ...
    def write(self, values):
        ...
        if 'templ_id' not in values and \
            ...
            self.env['cf.template.history'].create({
                ...
                "version": self.write_date.strftime('%Y%m%d%H%M%S'),
            })

        return super(CFTemplate, self).write(values)
